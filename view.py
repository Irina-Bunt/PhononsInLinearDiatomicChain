...
class view_window(QMainWindow, Ui_MainWindow):
    def __init__(self, parent=None):
        ...
        big_small_pixel_diff = 170
        self.controller = controller.controller(20)
        ...
        self.big_atoms_x0 = np.array([[begin_big_x0 + i * big_small_pixel_diff, 10] for i in range(5)])
        self.small_atoms_x0 = np.array([[begin_small_x0 + i * big_small_pixel_diff, 10] for i in range(5)])
    ...
    def main_loop_iteration(self):
        ...
        # self.controller.set_k(self.ui.WaveSlider.value() / 100)
        ...
        for current_index in range(self.big_atoms_x0.shape[0]):
            big_atoms_diff, small_atoms_diff, current_graph_circle_x = self.controller.get_atoms(current_index+1)
            ...
            circle = self.atom_scene.addEllipse(self.small_atoms_x0[current_index, 0] - 10 + small_atoms_diff, self.small_atoms_x0[current_index, 1] - 10, 20, 20, Qt.black)
            circle.setBrush(QBrush(QColor(255, 0, 0)))

        self.controller.update_t(self.ui.TimeSlider.value() / 1000)
    ...
    def update_parameters(self):
        ...
        # self.ui.WaveSlider.setRange(int(-(self.k_coef * np.pi) / (distance * 2)), int((self.k_coef * np.pi) / (distance * 2))) #
